...
export const generateSessionInsights = async (sessionData) => {
    try {
        const totalPoints = sessionData.stressPoints.length;
        const calmCount = sessionData.stressPoints.filter((p) => p.state === "calm").length;
        const vigilanceCount = sessionData.stressPoints.filter((p) => p.state === "vigilance").length;
        const tenseCount = sessionData.stressPoints.filter((p) => p.state === "tense").length;
        ...
        if (questionMap.size === 0) {
            console.log("No questions found in session data, attempting to fetch...");
            try {
        ...
        const questionImpactAnalysis = questionTimestamp.map((q) => {
            const questionTime = parseFloat(q.timestamp);
            const sevenSecondsLater = questionTime + 7.0;
            const stressAfterQuestion = sessionData.stressPoints.filter((p) => {
                const pointTime = p.timestamp / 1000;
                return pointTime >= questionTime && pointTime <= sevenSecondsLater;
            });
            const stressLevels = stressAfterQuestion.map((p) => p.state);
            const calmCount = stressLevels.filter((state) => state === 'calm').length;
            const vigilanceCount = stressLevels.filter((state) => state === 'vigilance').length;
            const tenseCount = stressLevels.filter((state) => state === 'tense').length;
        ...
        const questionImpactText = questionImpactAnalysis.map((q) => {
