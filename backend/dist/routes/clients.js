...
router.get("/:id", authMiddleware, async (req, res) => {
    try {
        const client = await Client.findById(req.params.id).populate("sessions");
        if (!client) {
    ...
    try {
        ...
        for (const client of clients) {
            for (const storedDescriptor of client.faceDescriptors) {
                const distance = calculateEuclideanDistance(faceDescriptor, storedDescriptor.descriptor);
